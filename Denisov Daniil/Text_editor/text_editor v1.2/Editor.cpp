...
void text_editor::logic(std::string _buffer)
{
  ...
  while (pos != _buffer.npos)//Пока не дошёл до конца буфера
  {
    ...
    if (pos != last_position)//Если не два знака препинания подряд
    {
      ...
      for (str = text_editor::fix_word.begin(); str != text_editor::fix_word.end();)
      {
        ...
        str++;
        if (str == text_editor::fix_word.end() && prioritet != GetMaxError())//Оно тут прячется
        {
          prioritet++;
...
bool text_editor::Get_file_info()
{
  ...
  int ask = 100;//Частота запроса просмотра слов у пользователя
  int temp = 1;//Выбор
  ...
  else
  {
    ...
    while (!fin.eof())
    {
      ...
      if (count > ask)
      {
        std::cout << "В файле много слов,вы уверены,что хотите видеть их все?(да - 1,нет - 0):";
        std::cin >> temp;
        if (!temp)
          break;
        ask += ask;
  ...
  std::cout << "Добавить эти слова в словарь?(да -1,нет - 0):";
  std::cin >> temp;
  fin.close();
  if (temp == 1)
  {
    Set_file_to_dictionary(path);
  }
  return true;
}
...
{
  int temp = 0;
  std::cout << "Нужен словарь по умолчанию?(1 - да,0 - нет):";
  std::cin >> temp;
  if (!temp)
  {
    fix_word.clear();
...
bool text_editor::Set_file_to_dictionary(std::string _path)
{
  std::ifstream fin;
  std::string str;
  fin.open(_path);
  ...
  else
  {
    ...
    std::cout << "Слова добавлены в словарь!\n";
  }
  fin.close();
  return true;
